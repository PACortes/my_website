...
Ahora bien, podríamos estar interesados en observaciones que cumplan criterios en una u otra variable. Por ejemplo, queremos trabajar con las observaciones de la especie **setosa** y todas aquellas que tengan un **Largo de Sépalo** mayor a 5.8 (independiente de la especie). Para aplicar el filtro, utilizaremos los operadores lógicos `x > y` y `x == y`, además del operador booleano `|`:



## 2.3 Filtrar observaciones que no contienen un valor en una variable
Cuando trabajamos con variables que contienen una gran cantidad de niveles, podríamos estar interesados en excluir algúnas de ellas para futuros análisis. Por ejemplo, queremos excluir las observaciones de las especies **setosa** y  **versicolor**. Para aplicar el filtro, utilizaremos los operadores lógicos `!` y ``%in%``, además del operador booleano `|`.
...
## 2.3 Filtrar observaciones que no contienen valores en múltiples variables
De igual forma, podríamos estar interesados en excluir observaciones que no cumplan condiciones en más de una varible. Por ejemplo, Por ejemplo, queremos excluir las observaciones de las especies **setosa** y  **versicolor** que tengan un **Largo de Sépalo** igual 5.8.
